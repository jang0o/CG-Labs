...
for i in range(len(model_v)):
    model_v[i][0], model_v[i][1], model_v[i][2] = turn(model_v[i][0], model_v[i][1], model_v[i][2], 0, pi/2, 0, ty=-0.03, tz=0.5)
...
def draw_polygon(model_v, model_f, image):
    ...
    for i in model_f:
        ...
        if np.dot(n, light_dir) < 0:
            ...
            if (n[2] < 0):
                color = (int(-100 * n[2]), int(-255 * n[2]), int(-200 * n[2]))
                draw_triangle(x0_2d, y0_2d, z0_3d, x1_2d, y1_2d, z1_3d, x2_2d, y2_2d, z2_3d, image, color, z_buffer)
